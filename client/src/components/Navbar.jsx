...
import { XIcon, MenuIcon, SearchIcon, TicketPlus, MapPin } from 'lucide-react';
...
const Navbar = () => {
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (search.trim()) {
      navigate(`/search?q=${encodeURIComponent(search.trim())}&city=${selectedCity}`);
      setSearch('');
    }
  };
        ...
        {!user ? (
          ...
          <div className='flex items-center gap-2'>
            <button
              ...
              className='flex items-center gap-1 px-3 py-1 text-sm bg-[#F84565] hover:bg-[#D63854] text-white rounded-full'
            >
              <TicketPlus size={14} /> My Bookings
            </button>

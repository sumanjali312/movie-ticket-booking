...
const SeatLayout = () => {
  ...
  // Parse query parameters from ShowTimes navigation
  ...
  const seatLayout = [
    {
      section: "Front Rows",
      rows: [
        { row: "A", seats: ["A1","A2","A3","A4","A5","A6","A7","A8","A9"] },
        { row: "B", seats: ["B1","B2","B3","B4","B5","B6","B7","B8","B9"] },
      ]
    },
    {
      section: "Middle Rows",
      rows: [
        { row: "C", seats: ["C1","C2","C3","C4","C5","C6","C7","C8","C9"] },
        { row: "D", seats: ["D1","D2","D3","D4","D5","D6","D7","D8","D9"] },
      ]
    },
    {
      section: "Premium Rows",
      rows: [
        { row: "E", seats: ["E1","E2","E3","E4","E5","E6","E7","E8","E9"] },
        { row: "F", seats: ["F1","F2","F3","F4","F5","F6","F7","F8","F9"] },
      ]
    }
  ];
      ...
      <div className="flex-1 flex flex-col items-center">
        ...
        {seatLayout.map((section, index) => (
          <div key={index} className="mb-10 w-full text-center">
            <h2 className="text-lg font-semibold mb-3 text-blue-700">{section.section}</h2>
            {section.rows.map((row) => (
              <div key={row.row} className="flex items-center justify-center gap-4 mb-2">
                <span className="w-6 text-gray-700 font-bold">{row.row}</span>
                <div className="flex gap-2 justify-center">
                  {row.seats.map((seatId) => {
                    const isSelected = selectedSeats.includes(seatId);
                    const locked = !selectedTime;
                    return (
                      <div
                        key={seatId}
                        ...
                        className={`w-10 h-10 flex items-center justify-center border rounded transition duration-200
                          ${locked
                            ...
                            : "bg-gray-100 text-gray-800 border-gray-400 hover:bg-red-500 hover:text-white cursor-pointer"
                          }`}
                      >
                        {seatId}
                      </div>
